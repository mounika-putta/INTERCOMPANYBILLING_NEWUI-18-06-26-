...
const ViewQuotationModal = ({ show, onClose, selectedInvoice }) => {
  ...
  return (
    <div className="quotation-view-overlay">
      <div className="quotation-view-modal">
        ...
        <div className="quotation-view-info">
          <div><span className="label">Quotation ID</span> : <span>{selectedInvoice.id}</span></div>
          ...
          <div><span className="label">Receiving</span> :<span>{selectedInvoice.receiving}</span></div>
          <div><span className="label">Status</span>: <span>{selectedInvoice.Status}</span></div>
          ...
          <tbody>
            {(selectedInvoice.details || []).map((d, i) => (
              <tr key={i}>
                ...
                <td style={{ textAlign: 'right' }}>{(d.quantity * d.unitRate)}</td>
                <td style={{ textAlign: 'right' }}>{d.discount || 0}</td>
                <td style={{ textAlign: 'right' }}>{(d.quantity * d.unitRate) - (d.discount)}</td>
                <td style={{ textAlign: 'right' }}>{d.tax}</td>
                <td style={{ textAlign: 'right' }}>
                  {(
                    (d.quantity * d.unitRate - d.discount) +
                    ((d.quantity * d.unitRate - d.discount) * d.tax) / 100
                  ).toFixed(2)}
                </td>
        ...
        <div className="quotation-view-info">
          <div><span className="label">Company Name</span> : <span>{selectedInvoice.receiving}</span></div>
